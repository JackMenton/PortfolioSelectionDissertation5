...
# Some Parameters
mu = 0.1 # drift of risky asset
sigma = 0.1 # volatility of risky asset
...
class PolicyNetwork(nn.Module):
    ...
    this network returns a mean and standard deviation - inputs are current 
    wealth and time left in investment horizon'''
...
class ValueFuncNetwork(nn.Module):
    ...
    and time left in investment horizon, outputs the approximated value of the 
    input state'''
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_returns):
        ...
        self.fc2 = nn.Linear(self.fc1_dims,self.fc2_dims)
        self.fc3 = nn.Linear(self.fc2_dims,n_returns) # output is value of the input state
        self.optimizer = optim.Adam(self.parameters(), lr = lr)
...
agent = Agent(alpha = 0.003, beta = 0.0035, input_dims = [2], gamma = 1, l1_size = 32, l2_size = 32)
score_mat = np.zeros(EPOCHS)
td_memory = np.zeros(EPOCHS)

# ---------- Training ---------- #
episode_scores = np.array([]) # array to store episode scores (value of inital state)
...
# PLots terminal wealths in testing phase
plt.figure()
